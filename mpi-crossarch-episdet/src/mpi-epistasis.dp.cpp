/**
 * mpi-crossarch-episdet: Multi-device and multi-node DPC++ application for high-order exhaustive epistasis detection using K2 scoring.
 *
...
__inline__ float blockReduceMin(float val, sycl::nd_item<3> item_ct1, float *shared) {  
    ...
    int group_size = item_ct1.get_local_range(2);
    ...
    for (int offset = group_size / 2; offset > 0; offset /= 2) {
        if (local_id < offset) {
            shared[local_id] = sycl::min(shared[local_id], shared[local_id + offset]);
        }
        item_ct1.barrier(sycl::access::fence_space::local_space);
    }
...
void sycl_mem_init(int datasetCases_size, int datasetControls_size, int combinationArray_size, int lgammaPrecalc_size, int output_size) try {
...
void sycl_mem_copy(unsigned long long *datasetCases, int datasetCases_size,
...
void sycl_launch_kernel(int numSNPs, int controlsSize, int casesSize, int numCombinationsPerKernelInstance, int CpuThreadID, sycl::queue *stream_id)
{
    ...
    stream_id->submit([&](sycl::handler &cgh) {  
        sycl::accessor<uint8_t, 1, sycl::access::mode::read_write,
            ...
            dpct_local_acc_ct1(sycl::range<1>(2 * WORKGROUP_SIZE * comb * sizeof(uint)), cgh);
        ...
        auto d_combinationArray_CpuThreadID_NUM_COMB_PER_KERNEL_INSTANCE_EPISTASIS_SIZE_ct2 =
            d_combinationArray + (CpuThreadID * NUM_COMB_PER_KERNEL_INSTANCE * EPISTASIS_SIZE);
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(
                sycl::range<3>(1, 1, blocksPerGrid) * sycl::range<3>(1, 1, WORKGROUP_SIZE),
...
int main(int argc, char *argv[]) try {
	...
	sycl_mem_init(datasetOnes_64packed_size, datasetZeros_64packed_size, NUM_COMB_PER_KERNEL_INSTANCE * NUM_CPU_THREADS * EPISTASIS_SIZE, sampleSize, output_size);	
	...
	sycl_mem_copy(datasetOnes_64packed, datasetOnes_64packed_size, datasetZeros_64packed, datasetZeros_64packed_size, NULL, 0, h_lgammaPrecalc, sampleSize, outputFromGpu, output_size);	
	...
	/* Creation of as many SYCL queues as the number of CPU threads used */
	std::vector<sycl::queue> syclQueues;
	for (int i = 0; i < NUM_CPU_THREADS; i++) {
    		syclQueues.emplace_back(sycl::queue(sycl::default_selector{}, sycl::property::queue::in_order()));
	}
	...
	for(unsigned long long j = mpi_world_rank; j < (unsigned long long) ceil(numCombinations / (double) NUM_COMB_PER_KERNEL_INSTANCE); j += mpi_world_size) {
		...
		syclQueues[omp_thread_id].memcpy( d_combinationArray + (omp_thread_id * NUM_COMB_PER_KERNEL_INSTANCE * EPISTASIS_SIZE),
		...
		syclQueues[omp_thread_id].memcpy(d_output + omp_thread_id, outputFromGpu + omp_thread_id, sizeof(float));

		sycl_launch_kernel(numSNPs, controlsSize, casesSize, numCombToGenerate, omp_thread_id, &(syclQueues[omp_thread_id]));
		
		syclQueues[omp_thread_id].memcpy(outputFromGpu + omp_thread_id, d_output + omp_thread_id, sizeof(float));
		syclQueues[omp_thread_id].memcpy(output_indexFromGpu + omp_thread_id, d_output_index + omp_thread_id, sizeof(int));

		syclQueues[omp_thread_id].wait();
	...
	/* Reduce scores and get the indices of best score */
