...
__inline__ __device__ float shfl_xor_64(float scalarValue, const int n)
{
...
__inline__ __device__ float warpReduceMin(float val)
{
        ...
        val = min(val, shfl_xor_64(val, 1));
        val = min(val, shfl_xor_64(val, 2));
        val = min(val, shfl_xor_64(val, 4));
        val = min(val, shfl_xor_64(val, 8));
        val = min(val, shfl_xor_64(val, 16));
        val = min(val, shfl_xor_64(val, 32));
...
__inline__ __device__ float warpReduceMax(float val) {
	val = max(val, shfl_xor_64(val, 1));
	val = max(val, shfl_xor_64(val, 2));
	val = max(val, shfl_xor_64(val, 4));
	val = max(val, shfl_xor_64(val, 8));
	val = max(val, shfl_xor_64(val, 16));
        val = max(val, shfl_xor_64(val, 32));
	return val;
}
...
__inline__ __device__ int shfl_xor_64(int scalarValue, const int n)	// Specialization for 'int' reduction
{
        return __shfl_xor(scalarValue, n, 64);
...
__inline__ __device__ int warpReduceSum(int val) {
        val += shfl_xor_64(val, 1);
        val += shfl_xor_64(val, 2);
        val += shfl_xor_64(val, 4);
        val += shfl_xor_64(val, 8);
        val += shfl_xor_64(val, 16);
        val += shfl_xor_64(val, 32);
        return val;
}
